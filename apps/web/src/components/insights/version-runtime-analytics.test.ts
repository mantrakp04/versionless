...
test("says transforms are absent rather than printing 0.0", () => {
  expect(depthQualifier([route({ avgDepth: 0 })], 0)).toBe("no transforms");
  expect(depthQualifier([route({})], 1.4)).toBe("1.4 transforms per request");
});
