...
const STREAM_COMPILE_INTERVAL_MS = 120;
  ...
  const startCompilation = useCallback((source: string) => {
    ...
    void compile(normalizedSource, {
      ...
      .finally(() => {
        ...
        const delay = latest.streaming
          ? Math.max(
              0,
              STREAM_COMPILE_INTERVAL_MS -
                (performance.now() - lastStartedAt.current),
            )
          : 0;
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground text-[0.625rem]">
          {import.meta.env.DEV && content.error instanceof Error
