 ...
 *
 *   bun run --cwd apps/web validate-queries
 *
...
const CASES: Array<{
  ...
  { name: "trace-spans", options: traceSpansQueryOptions(projectId, occurrence.traceId) },
 ...
 * `apps/web` taking on a ClickHouse client dependency it otherwise has no use
 * for.
 */
async function run(
  query: string,
...
for (const testCase of CASES) {
  const { query, params } = sqlOf(testCase.options);
  try {
