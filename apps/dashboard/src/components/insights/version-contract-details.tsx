...
export function buildContractHeadline(
  ...
  if (!previous) {
    return versions.length > 1
      ? `${detail.version} is the oldest saved version.`
      : `${detail.version} is the only saved version.`;
  }
  const delta = detail.endpointCount - previous.endpointCount;
  if (delta === 0) {
    return `Compared with ${previous.version}, this version has the same number of API endpoints.`;
  }
  return `Compared with ${previous.version}, this version ${
    delta > 0 ? "adds" : "removes"
  } ${Math.abs(delta)} API ${plural(Math.abs(delta), "endpoint")}.`;
}
...
export function shapeVerdict(
  ...
  if (!oldest || ordered.length < 2) {
    return { verdict: "Only saved", tone: "muted" };
  }
  if (oldest.version === detail.version) {
    return { verdict: "Oldest saved", tone: "muted" };
  }
  const growth = detail.endpointCount - oldest.endpointCount;
  if (growth > 0) return { verdict: "More endpoints", tone: "positive" };
  if (growth < 0) return { verdict: "Fewer endpoints", tone: "negative" };
  return { verdict: "Same size", tone: "neutral" };
}
...
export function inventoryVerdict(
  ...
  if (summary.introduced > 0)
    return { verdict: "Endpoints added", tone: "positive" };
      ...
      <AccordionTrigger className="px-4 py-3.5 hover:no-underline">
        <div className="grid min-w-0 flex-1 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 sm:grid-cols-[minmax(0,1fr)_5rem_6rem_7rem]">
          <div className="flex min-w-0 items-center gap-3">
            <Badge
              className={`font-mono ${endpointBadgeClass(endpoint)}`}
              variant="outline"
            >
      ...
      <AccordionContent className="px-4 pb-4">
        <div className="overflow-hidden rounded-md border bg-muted/15">
          <div className="grid grid-cols-2 divide-x border-b bg-muted/35 sm:grid-cols-4">
            ...
            />
            <EndpointFact label="Saved shapes" value={shapes} />
            <EndpointFact
              label="Input fields"
  ...
  if (detail.endpointDetails.length === 0) {
    return (
      <div className="rounded-lg border border-dashed px-4 py-8 text-center text-muted-foreground">
        No endpoints were included in this saved version.
      </div>
    );
  }
...
function SurfaceEvolution({ versions }: { versions: ProjectVersionDetail[] }) {
  ...
  return (
    <ReportPanel
      className="relative"
      description="Endpoints and data models across every saved version."
      title="API size over time"
    >
...
function SurfaceMix({ detail }: { detail: ProjectVersionDetail }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <ReportPanel
        description="How the HTTP endpoints are distributed."
        title="HTTP methods"
      >
...
export function VersionContractDetails({
  ...
  const definitionIndex = (offset: number) =>
    String(runtimeSectionCount + offset).padStart(2, "0");
  ...
  return (
    <Report defaultExpanded={defaultExpanded}>
      ...
      <ReportGroupHeading>Version contents</ReportGroupHeading>
      ...
      <div className="pb-4">
        ...
        >
          ...
          />
          <ReportHeadlineStat label="Data models" value={detail.modelCount} />
          <ReportHeadlineStat
            label="Data fields"
      ...
      <ReportSection
        ...
        title="What this version exposes"
      ...
      <ReportSection
        ...
        verdict={detail.provenance?.repo ? "Linked" : "Unlinked"}
          ...
          <div>
            ...
            {detail.provenance ? (
              ...
              <div className="text-muted-foreground">
                No source repository information recorded.
              </div>
            )}
          </div>
        </ReportPanel>
