...
/** Keeps generated dashboards responsive without failing normal multi-widget loads. */
export class SandboxQueryScheduler {
  private active = 0;
  private readonly pending: Array<() => void> = [];

  constructor(
    private readonly maxConcurrent = MAX_CONCURRENT_QUERIES,
    private readonly maxPending = MAX_PENDING_QUERIES,
  ) {}

  run<TResult>(task: () => Promise<TResult>): Promise<TResult> {
    if (this.pending.length >= this.maxPending) {
      return Promise.reject(
        new Error("This dashboard requested too many queries."),
      );
    }
...
export function MdxMessage({
  ...
  useEffect(() => {
    let active = true;
    const queryScheduler = new SandboxQueryScheduler();
  ...
  return (
    <iframe
      ...
      src={`${env.BASE_URL}chat-sandbox.html`}
      style={{ height }}
