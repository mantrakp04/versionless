...
function errorLogsSql(options?: {
  ...
  return `WITH toUInt16OrZero(
       LogAttributes['http.response.status_code']
     ) AS status
SELECT ${options?.bucketExpression ? `${options.bucketExpression} AS bucket,` : ""}
...
function errorTraceRootsSql(): string {
  return `WITH parseDateTime64BestEffortOrNull(
