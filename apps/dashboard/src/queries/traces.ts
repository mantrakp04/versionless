...
export function traceListQueryOptions(input: {
  ...
  ${input.version ? "WHERE SpanAttributes['versionless.version'] = {version: String}" : ""}
  GROUP BY TraceId
),
selected_roots AS (
...
export function traceSpansQueryOptions(
  projectId: string,
